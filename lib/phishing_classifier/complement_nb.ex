defmodule PhishingClassifier.ComplementNB do
  @moduledoc """
  ...
  This builds directly on a trained `%PhishingClassifier.NaiveBayes{}`
  (it reuses its per-class word counts), so pruning / balancing options carry over.
  """

  alias PhishingClassifier.Tokenizer
  ...
  @doc "Build a WCNB model from a trained `%PhishingClassifier.NaiveBayes{}`."
  @spec from_model(PhishingClassifier.NaiveBayes.t()) :: t()
  def from_model(%PhishingClassifier.NaiveBayes{} = m, opts \\ []) do
    v = m.vocab_size
    alpha = m.alpha
  ...
  def train(examples, opts \\ []) do
    examples |> PhishingClassifier.NaiveBayes.train(opts) |> from_model()
  end
  ...
  def classify(model, text, opts \\ []) do
    positive = Keyword.get(opts, :positive, :phish)
    negative = Keyword.get(opts, :negative, :safe)
