defmodule PhishingClassifier.Evaluation do
  @moduledoc """
  ...
  @spec evaluate(module_or_any :: any(), [example()], (any(), String.t() -> atom()), atom()) ::
          map()
  def evaluate(model, examples, classify_fn, positive \\ :phish) do
  ...
  def report(m) do
    ...
    ── Evaluation (#{m.total} test docs) ──────────────
      ...
      Precision: #{pct(m.precision)}   (of flagged-phishing, how many really were)
      Recall   : #{pct(m.recall)}   (of truly-phishing, how many we caught)
      F1       : #{pct(m.f1)}

      Confusion matrix
                     pred phish   pred safe
        act phish        #{pad(c.tp)}        #{pad(c.fn)}
        act safe         #{pad(c.fp)}        #{pad(c.tn)}
    ───────────────────────────────────────────────────
    """)
  end
