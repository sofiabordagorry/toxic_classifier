defmodule ToxicClassifier.Tokenizer do
  @moduledoc """
  Turns raw text into a list of feature tokens for the Naive Bayes classifier.

    * lowercases and normalises common leetspeak obfuscation (`f4ck`, `sh1t`)
    * collapses character floods (`loooool` -> `lool`, `!!!!!` -> `!!`)
    * keeps toxicity *signals* that plain tokenisers throw away:
        - `__allcaps__` when a word is SHOUTING
        - `!` and `?` as their own tokens
    * emits both unigrams and bigrams, so "you are" + "are stupid" carry the
      phrase-level context a bag-of-words would miss.
  """
